...
const PROXY_URL = 'http://localhost:5000/api/send-email';
...
export const sendWelcomeEmail = async (toEmail: string, toName: string) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || "Email proxy error");
    }

    return true;
  } catch (error) {
    console.error("Error sending welcome email via proxy:", error);
...
export const sendOrderEmail = async (toEmail: string, customerName: string, orderId: string, totalAmount: number) => {
  ...
  try {
    ...
    console.error("Error sending order email via proxy:", error);
